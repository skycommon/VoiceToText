...
> **Download**: Get `VoiceToText-1.0.0-windows.zip` from [Releases](https://github.com/skycommon/VoiceToText/releases), unzip it, then double-click `VoiceToText-Setup-1.0.0.exe` to install.
...
The app itself runs locally (UI, recording, denoise); recognition is performed by online services, so the installer is small (~27 MB) while Chinese accuracy stays high.

## 1. Install

- **Installer (recommended, in Releases)**: download `VoiceToText-1.0.0-windows.zip`, unzip to get `VoiceToText-Setup-1.0.0.exe`, run it (installs to the user program folder, no admin rights required). Creates Start Menu and desktop shortcuts.
...
On first launch, open **Settings** and enter the credentials for one service, then save. **Use whichever service you have a key for**; all three can be switched anytime from the dropdown.
...
5. **Separate speakers**: only available for the "iFlytek Transcription" backend (auto-disabled for others). When checked, results are split by speaker, e.g. `Speaker 1: …` `Speaker 2: …`. For meetings, also set "Settings → Speaker count".
...
- **Accuracy vs size**: recognition is done by online services (iFlytek / Baidu / OpenAI are all high-accuracy models); the app ships no local speech model, so the installer is only ~27 MB — far smaller than local Whisper models (hundreds of MB to GB).
...
│  └─ VoiceToText-Setup-1.0.0.exe       # installer (generated by installer.iss)
