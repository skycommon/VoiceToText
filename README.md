...
本地只跑界面、录音与降噪，识别走线上服务，所以安装包体积小（约 24 MB），中文识别精度高。
...
首次打开后，点「设置」填入对应服务的密钥并保存。**哪家有 Key 用哪家**，四者（讯飞听写 / 讯飞转写 / 百度语音 / OpenAI）可随时在界面下拉切换。
...
5. **区分说话人**：仅「讯飞转写」后端可用（其它后端自动置灰）。勾选后，转写结果按说话人分段，形如 `说话人1：……` `说话人2：……`。开会时说话人数量默认为 2，可在本机配置文件 `%APPDATA%\VoiceToText\config.json` 的 `speaker_number` 字段调整。
...
- **精度 vs 体积**：识别由线上服务完成（讯飞 / 百度 / OpenAI 均为高精度大模型），本地不含语音模型，因此安装包仅约 24 MB，远小于本地 Whisper 模型（数百 MB～GB 级）。
